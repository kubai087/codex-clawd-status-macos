...
LEGACY_COMMAND_MARKERS = (
    "/codex-clawd-status/scripts/codex_clawd_hook.py",
    "\\codex-clawd-status\\scripts\\codex_clawd_hook.py",
)
...
def _is_managed(command: object, current: str) -> bool:
    if not isinstance(command, str):
        return False
    return command == current or any(marker in command for marker in LEGACY_COMMAND_MARKERS)
...
def _remove_managed_entries(entries: list[Any], command: str) -> list[Any]:
    ...
    for entry in entries:
        ...
        remaining = [
            ...
            if not (isinstance(hook, dict) and _is_managed(hook.get("command"), command))
        ]
        if remaining:
...
def merge_hooks(data: dict[str, Any], command: str) -> dict[str, Any]:
    ...
    for event in HOOK_EVENTS:
        ...
        hooks[event] = [*_remove_managed_entries(entries, command), _entry(command, event)]
    return result
